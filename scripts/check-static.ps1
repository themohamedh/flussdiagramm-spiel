...
$staticServerSource = Read-Utf8 "scripts/serve-static.mjs"
...
Assert-Condition ($packageJson.scripts.test -eq "node --test tests/static-integrity.test.mjs && node --test Flussdiagramm/tests/static-integrity.test.mjs") "package.json test script should cover root and Flussdiagramm static tests"
Assert-Condition ($packageJson.scripts.'test:all' -eq "npm test && npm run test:e2e") "package.json must expose the combined static and E2E suite"
...
Assert-Condition ([regex]::IsMatch($staticServerSource, 'const relative = path\.relative\(root, target\);')) "Static server should compare resolved paths with path.relative"
Assert-Condition ([regex]::IsMatch($staticServerSource, 'relative === "\.\."')) "Static server should reject the parent path"
Assert-Condition ([regex]::IsMatch($staticServerSource, 'relative\.startsWith\(`\.\.\$\{path\.sep\}`\)')) "Static server should reject paths below the parent"
Assert-Condition ([regex]::IsMatch($staticServerSource, 'path\.isAbsolute\(relative\)')) "Static server should reject absolute relative-path results"
Assert-Condition (-not [regex]::IsMatch($staticServerSource, 'target\.startsWith\(root\)')) "Static server should not use a vulnerable string-prefix containment check"
...
$setGameModeBody = Get-BalancedExpression $html "function setGameMode(" "{" "}"
...
Assert-Condition ([regex]::IsMatch($setGameModeBody, 'const modeChanged = nextMode !== currentMode;')) "Mode switching should detect a real mode change"
Assert-Condition ([regex]::IsMatch($setGameModeBody, 'if \(modeChanged\) resetAll\(\);')) "Mode switching should reset the previous attempt"
...
Assert-Condition ([regex]::IsMatch($toniSource, 'window\.addEventListener\("tarif-toni:request-state", emitState\);')) "Toni should answer state requests from the host page"
Assert-Condition ([regex]::IsMatch($html, 'setGameMode\("learn"\);\s*window\.dispatchEvent\(new CustomEvent\("tarif-toni:request-state"\)\);')) "Initial host state should request persisted Toni preferences"
Assert-Condition (-not [regex]::IsMatch($html, 'setGameMode\("learn"\);\s*setToniEnabled\(true\);')) "Initial host state must not overwrite persisted Toni preferences"
...
Assert-Condition ([regex]::IsMatch($e2eSource, 'mode-isolation')) "E2E suite should cover mode-state isolation"
Assert-Condition ([regex]::IsMatch($e2eSource, 'toni-preferences')) "E2E suite should cover persisted Toni preferences"
Assert-Condition ([regex]::IsMatch($e2eSource, 'path-boundary')) "E2E suite should cover the local server path boundary"
...
Write-Host "- Mode changes start isolated attempts and Toni preferences survive reloads."
...
Write-Host "- Navigation caching and local-server path containment are checked."

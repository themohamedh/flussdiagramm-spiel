...
test("Tarif Toni AI remains free, source-bound, and disabled in exam mode", () => {
  ...
  assert.match(toniApiSource, /setTimeout\(\(\) => controller\.abort\(\), 22_000\)/, "The server must allow slow zero-data-retention providers enough time");
  ...
  assert.doesNotMatch(
    toniApiSource,
    /if\s*\(\s*openRouterResponse\.status\s*===\s*404\s*\)/,
    "A ZDR 404 must fall back locally instead of retrying without ZDR",
  );
